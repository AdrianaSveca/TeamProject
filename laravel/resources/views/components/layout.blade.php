                ...
                <div class="flex h-16 items-center">
                    ...
                    <div class="-mr-2 flex md:hidden">
                        <button @click="open = ! open" type="button" class="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-white/5 hover:text-white focus:outline-none">
                            ...
                            <svg class="h-6 w-6 hidden" :class="{'hidden': ! open, 'block': open }" stroke="currentColor" fill="none" viewBox="0 0 24 24">
            ...
            <div :class="{'block': open, 'hidden': ! open}" class="hidden md:hidden">
                <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
                    ...
                    <button 
                                id="theme-toggle" 
                                type="button" 
                                class="text-gray-300 hover:bg-white/10 hover:text-[#7FA82E] focus:outline-none rounded-lg text-sm p-2.5 ml-2"
                                x-data="{ 
                    ...
                    @auth
                        ...
                        @if(Auth::user()->role === 'admin')
                            <a href="{{ route('admin.dashboard') }}" class="block rounded-md px-3 py-2 text-base font-medium transition duration-150 ease-in-out {{ request()->routeIs('admin.dashboard') ? 'bg-[#2B332A] text-[#7FA82E] font-bold' : 'text-gray-300 hover:bg-white/5 hover:text-[#7FA82E]' }}">Admin Panel</a>
                        @endif
                    @else
                        <a href="{{ route('login') }}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-[#7FA82E] transition duration-150 ease-in-out">Log in</a>
                        <a href="{{ route('register') }}" class="block rounded-md px-3 py-2 text-base font-medium transition duration-150 ease-in-out {{ request()->is('register') ? 'text-[#7FA82E] font-bold' : 'text-gray-300 hover:bg-white/5 hover:text-[#7FA82E]' }}">Join Us</a>
                    @endauth
                </div>
                ...
                <div class="border-t border-white/10 pt-4 pb-3">
                    <div class="flex items-center px-5">
                        <div class="ml-3 mr-auto">
                            <div class="text-base font-medium leading-none text-[#678924]">{{ Auth::user()->name }}</div>
                            <div class="text-sm font-medium leading-none text-[#678924] mt-1">{{ Auth::user()->email }}</div>
                        </div>
                    </div>
                    <div class="mt-3 space-y-1 px-2">
                        <a href="{{ route('profile.edit') }}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-[#7FA82E]">Your Profile</a>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <a href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();" class="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-white/5 hover:text-[#7FA82E]">Log Out</a>
                        </form>
                    </div>
                </div>
                @endauth
