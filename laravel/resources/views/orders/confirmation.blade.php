        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white p-8 rounded-2xl shadow-md">
                    ...
                    @foreach($order->items as $item)
                        <div class="flex justify-between py-4 items-center">
                            <div class="flex items-center gap-4">
                                <div class="w-16 h-16 bg-gray-100 rounded-xl overflow-hidden">
                                    @if($item->product->product_image)
                                        <img src="{{ asset($item->product->product_image) }}" class="w-full h-full object-cover" alt="{{ $item->product->product_name }}">
                                    @else
                            ...
                            <div class="text-right">
                                <p class="font-bold">£{{ number_format($item->order_item_quantity * $item->order_item_price, 2) }}</p>
                ...
                <div class="flex justify-between text-gray-900 font-bold text-lg">
                    <span>Total</span>
                    <span>£{{ number_format($order->items->sum(fn($i) => $i->order_item_quantity * $i->order_item_price), 2) }}</span>
                </div>
